...
- `modSGQInterface.bas` (Cleaned of mojibake)
- `modConstants.bas` (Restored from stable backup)
- `modExcelUtils.bas` (Restored from import_backup_20251024 - clean, contains SetNamedValue)
- `modWorkbookHandlers.bas` (Reset to minimal state due to corruption in backups)
- `modSGQAdministration.bas` (Restored from stable backup and patched for compilation)

### Manual Reconstruction
- `modGraphClient.bas`: Reconstructed from available code snippets and logic.
- `modSGQAdministration.bas`: Reconstructed from logical fragments and user screenshots due to universal corruption in available backups and Git history.
- `modWorkbookHandlers.bas`: Reset to minimal state to guarantee compilation.

## 3. Encoding Repair
- Confirmed `modSGQInterface.bas` is now displaying "Système" correctly.
- Created robust repair script `scripts/fix-mojibake.ps1` for future targeted use.
