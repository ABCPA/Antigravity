...
De plus, le fichier `manifest.json` actuel est statique et incomplet (il manque les nouveaux modules de test). Il doit devenir la source de vérité dynamique générée par le système.

## Changements Proposés

### 1. Consolidation et Nettoyage

**Critique :** Le module `modSGQTrackingBuilder` est identifié comme "Fantôme" (présent dans Excel, absent du disque) et cause la double génération de fichiers.

**Action Immédiate :**
- [ ] Supprimer `modSGQTrackingBuilder` du classeur (via script PowerShell).
- [ ] Retirer la référence dans `modSGQAdministration` (`modulesToToggle`).
- [ ] Supprimer du `manifest.json`.

**Action de Consolidation :**
- [ ] Déplacer les modules de `xvba_modules/` vers `vba-files/Module/`.

**Modules à déplacer :**
- `modAnalyticCalc.bas`, `modAnalyticETL.bas`, `modAnalyticReporting.bas`
- `modCQ...` (Reporting, Rules, Scanner)
- `modInspection...` (Audit, Report)
- `modTest_AnalyticalBug.bas`

### 2. Standardisation du Manifeste

**Action :** Mettre à jour `manifest.json` pour refléter tous les modules du projet, y compris les tests.
...
- [ ] Compiler le projet après migration (`test-vbide-and-compile.ps1`)
