...
- **[Dockerfile](file:///c:/VBA/SGQ%201.65/docker-mcp/m365/Dockerfile)**: Now generates an internal `entrypoint.sh` with Linux-compatible line endings. This script creates a symlink from `/root/.m365rc` to the persistent volume mount point, ensuring tokens are saved and restored across container restarts.
...
3.  **Confirm persistence**:
    - Once the script says "Authentification réussie", you can restart Docker (`launch-docker-mcp.ps1`) and the toolkit will start without prompting for login.
